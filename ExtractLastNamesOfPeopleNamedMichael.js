...
function getMichaelLastName(inputText) {
  ...
  for (let i = 0; i < strArr.length; i++) {
    if (strArr[i] === "Michael" && regexUpCase.test(strArr[i + 1].charAt(0))) {
      let lastName = strArr[i + 1];
      if (regexPunc.test(lastName)) {
        lastName = lastName.replace(regexPunc, "");
      }
      michaels.push(lastName);
    }
  }
  console.log(michaels);
  return michaels;
}
